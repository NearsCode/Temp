        ...
        content: `
          <div class="layui-form" lay-filter="filter-test-layer" style="width:360px;margin: 16px auto 0;">
            <div class="demo-login-container">
                ...
                <div>下载器一定要配置好Ua和端口: <a style="color:green;" target="_blank" href="https://docs.qq.com/doc/DWnlpY2pkclpuUEFX?no_promotion=1">点击查看配置说明</a></div>
                <div>不限次数pc网页稳定版: <a style="color:green;" target="_blank" href="https://www.aifenxiang.net.cn/">点击前往</a></div>
            ...
            .then((res) => {
              ...
              if (res.code == 200) {
                ...
                if (res.data > 100) {
                  ...
                  setTimeout(() => {
                    $('#parse').html('<p>解析</p>');
                    layer.alert('解析次数已达上限，不限次数稳定版！地址：https://www.aifenxiang.net.cn', {
                      title: '提示',
                    });
                  }, 3000);
                } else if (res.data == 50) {
      ...
      .then((res) => {
        if (res.code == 200) {
          ...
          if (size > 5242880000) {
            ...
            $('#parse').html('<p>解析</p>');
            swal({
              text: '文件大于5G,插件暂不支持下载,请前往PC网页版下载！',
              icon: 'warning',
            });
            return false;
          }
          ...
          GM_xmlhttpRequest({
            ...
            onload: function (response) {
              ...
              if (responseData.code !== 200) {
                ...
                layer.close(laysermsg);
                $('#parse').html('<p>解析</p>');
                config.url = responseData.data.urls[0].url;
                config.ua = responseData.data.ua;
